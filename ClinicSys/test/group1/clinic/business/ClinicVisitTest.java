...
public class ClinicVisitTest {
	...
	private static void testGetAndSetComplaint() {
		...
		testGetAndSetComplaint("Case 1: Valid Data - complaint without leading/trailing spaces (This is a complaint)", "This is a complaint", "This is a complaint");
		testGetAndSetComplaint("Case 2: Invalid Data - complain with leading/trailing spaces (   This is a complaint   )", "   This is a complaint   ", "This is a complaint");
	...
	private static void testGetAndSetComplaint(String testCase, String complaint, String expectedComplaint){
		...
		try{
			...
			if (aVisit.getComplaint().toString().equalsIgnoreCase(expectedComplaint))
				out.println("The expected complaint is: " + expectedComplaint + "The complaint returned is: " + aVisit.getComplaint() + "\n");
			else
			...
			out.println("::: EROOR ::: TEST FAILED UNEXPECTED EXCEPTION TYPE! " + e.getClass() + " "
					+ e.getMessage());
		}
	...
	private static void testGetPatient() {
		...
		try{
			...
			out.println("Patient's name: " + testPatient.getName().getFullName() +
					"\nPatient's gender: " + testPatient.getGender() + 
	...
	private static void testGetAndSetPriority(String testCase, Priority aPriority, String expectedPriority, int expectedPriorityCode) {
		out.println(testCase + "\n");
		...
		try{
			...
			if (aVisit.getPriority().getCode() == expectedPriorityCode)
				out.println("The expected priority value is: " + expectedPriority + " The priority value returned is: " + aVisit.getPriority());
			else
	...
	private static void test5ParamSetAndGetRegistrationDateAndTime(String testCase, int year, int month, int day, int hour, int sec) {
		out.println(testCase +"\n");			
		...
		try{
			...
			out.println("The expected resgistration date and time is: " + year + "-" + month + "-" + day + "T" + hour + ":" + sec/60
					+ " The registration date and time returned is:" + aVisit.getRegistrationDateAndTime());
		}
		catch (IllegalArgumentException iae){
			out.println("::: ERROR ::: TEST FAILED");
	...
	private static void test5ParamSetAndGetTriageDateAndTime(String testCase, int year, int month, int day, int hour, int sec) {
		out.println(testCase +"\n");			
		...
		try{
			...
			out.println("The expected triage date and time is: " + year + "-" + month + "-" + day + "T" + hour + ":" + sec/60
					+ "The triage date and time returned is:" + aVisit.getTriageDateAndTime());
		}
		catch (IllegalArgumentException iae){
			out.println("::: ERROR ::: TEST FAILED");
