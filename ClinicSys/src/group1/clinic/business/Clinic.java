...
public class Clinic extends Observable implements PatientVisitManager {
	 ...
	 * 
	 * @param patientConnection
	 *            existing connections to Patient database.
	 * @param visitConnection
	 *            existing connection to Visit database.
	 * @param factory
	 *            connection to existing factory type object.
	 *
	 */
	 ...
	 * 
	 * Throws IOException if there is a problem when saving any of the
	 * databases.
	 * 
	 */
	@Override
	...
	/**
	 * Creates a visit for a provided patient with a given complaint. Afterwards
	 * adds the visit into the database.
	 * 
	 * @param patient
	 *            Existing Patient type order for which a visit needs to be
	 *            created.
	 * @param complaint
	 *            complaint that the patient has.
	 */
	@Override
	...
	/**
	 * Looks through the patientConnection database to find a patient which has
	 * a matching ramq number.
	 * 
	 * Throws NonExistingPatientException if the patient can not be found in the
	 * database.
	 * 
	 * @param ramq
	 *            String representation of the ramq.
	 * 
	 */
	@Override
	 ...
	 * 
	 * @param meds
	 *            Medication that will be used to search for patients.
	 * @return List<Patient> List of Patients which all use the given
	 *         medication.
	 */
	@Override
	...
	/**
	 * First creates a new patient. Then sets additional information about the
	 * patient. When all this is done adds the patient to the patientConnection
	 * database.
	 * 
	 * @param firstName
	 ...
	 * @param ramq
	 *            Ramq number of the patient.
	 * @param telephone
	 *            Telephone of the patient.
	 * @param meds
	 *            Medication that the patient is taking.
	 * @param conditions
	 *            Conditions that the patient exhibits.
	 * 
	 */
	@Override
	public void registerNewPatient(String firstName, String lastName, String ramq, String telephone, Medication meds,
		...
		if (ramq == null || ramq.length() != 12)
			throw new IllegalArgumentException(
					"Clinic.registerNewPatient() - Ramq is null or has invalid amount of characters.");
	...
	/**
	 * Checks if the ramq is null, if the ramq contains 12 characters. Then
	 * looks at the first 4 characters of the ramq to look if they are alphabet
	 * characters. Then looks if the last 8 characters are digits.
	 * 
	 * Throws IllegalArgumentException If any of the conditions are not met.
	 * 
	 * @param ramq
	 *            String representation of a ramq which will be checked.
	 */
	private void validateRamq(String ramq) {

		if (ramq == null || ramq.length() != 12)
			throw new IllegalArgumentException(
					"Clinic.findPatient() - Ramq string is either null or does not contain 12 characters.");
		...
		for (int i = 0; i < 4; i++)
			if (!(Character.isAlphabetic(letters.charAt(i))))
				throw new IllegalArgumentException(
						"Clinic.findPatient() - Ramq contains digits in the first 4 characters.");
